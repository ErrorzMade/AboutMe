...
As you can see, it looks a lot more refined and hopefully more readable than the original design. Please keep in mind not all the maps are finished hence why some maps say no plans, but they will all be finished by the time the update launches. All the maps are broken down into Small, Medium and Large with indicators on how many floors each have. Diving into the map viewer its self, this is what it now looks like.
...
In terms of non map related updates, I have reworked the module system. Before It was quite simple and confusing at the same time, and it wasn't quite as customisable as I wanted it to be. I had been meaning to fix it for a while, so I'm happy it's finally done. This is what it looks like before vs after.
...
And that brings us to the end of Preview 2! I hope to get the update out at some point before the end of the month, but that's more of a vague timeframe I'm aiming for. I won't set any exact date, it'll come out when its ready. Just hopefully it's ready this month. I have to finish the maps, polish up the UI just a tiny but more and most importantly add support for PhasOverlay Web, which hopefully won't take long at all. When the update comes out I will push a detailed patch notes blogpost that goes over all of the changes, so be on the lookout for that! Thanks for reading.